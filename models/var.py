...
class VAR(nn.Module):
    ...
    @torch.no_grad()
    def autoregressive_infer_cfg_get_fhats(
        self,
        B: int,
        label_B: Optional[Union[int, torch.LongTensor]],
        g_seed: Optional[int] = None,
        ...
        f_hats = [
            self.vae_proxy[0].fhat_to_img(f_hat).add_(1).mul_(0.5) for f_hat in f_hats
        ]

        return f_hats  # de-normalize, from [-1, 1] to [0, 1]
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            x_BLC += (
                self.lvl_embed(self.lvl_1L[:, :ed].expand(B, -1)) + self.pos_1LC[:, :ed]
            )  # lvl: BLC;  pos: 1LC
        ...
        if self.prog_si == 0:
            if isinstance(self.word_embed, nn.Linear):
                x_BLC[0, 0, 0] += (
                    self.word_embed.weight[0, 0] * 0 + self.word_embed.bias[0] * 0
                )
            else:
                s = 0
